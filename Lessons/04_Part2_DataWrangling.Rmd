...
NTL.nutrient.data.PeterPaul <- 
  ...
  mutate(month = month(sampledate)) %>% # mutate to get column for month
  select(lakeid:daynum, month, sampledate:comments) %>% # reorder columns
  drop_na(depth) #dropping all the rows that contain nas in depth
...
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #what is the difference from filter then?
