...
`&` #aggregate if both are true
`|` #if only one or the other is true

```{r filtering data}
class(NTL.phys.data$lakeid) #shows up as factor instead of character (strings as factors)(note: she fixes it later)
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #cleaner way
...
NTL.phys.data.tues <- filter (NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: Because the same year is repeated multiple times
...
#1998-07-16, East Long Lake, 0.5 - high temp of 34.1
...
```{r using pipes}
NTL.phys.data.processed <- #ask if there is a keyboard shortcut for pipes
  NTL.phys.data %>%
...
Long file that you can easily get lost in
